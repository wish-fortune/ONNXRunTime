...
__global__ void PickGptPastStateKernelImpl(
  ...
  int input_offset = (layer_id * block_per_layer) + (is_v * block_per_past_state) +
                     (beam_indices[batch_beam_id] * block_per_beam) +
                     (head_id * block_per_head) + (seq_id * head_size);

  int output_offset = (layer_id * block_per_layer) + (is_v * block_per_past_state) +
                      (batch_beam_id * block_per_beam) +
                      (head_id * block_per_head) + (seq_id * head_size);
...
template bool PickGptPastStateKernel(
    float* past_state,
    const float* present_state,
    const int32_t* beam_indices,
    int batch_beam,
